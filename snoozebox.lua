...
local function start_of_day(timestamp)
   ...
   local sod_timestamp = os.time({year=parts.year,
                                  month = parts.month,
                                  day = parts.day,
                                  hour = 0})
...
function snoozebox.go_to_sleep(config)
   ...
   for _, mailbox in ipairs(mailboxes) do
      local sleep_count, interval = string.match(mailbox, config["base_folder"] .. "/Snooze_(%d+)([hdmwy]?)")
      if interval == "h" then mult = 3600
         elseif interval == "d" then mult = 86400
         elseif interval == "w" then mult = 604800
         elseif interval == "m" then mult = 2592000
         elseif interval == "y" then mult = 31536000
         else mult = 1
      end
      ...
      for _, message in ipairs(msgs) do
         ...
         new_text = add_snooze_header(remove_snooze_headers(text), wake_time)
         table.insert(flags, config["snoozed_tag"])
         if account[config["base_folder"] .. "/Snoozed"]:append_message(new_text, flags, date) then
...
function snoozebox.wake_up(config)
   ...
   for _, message in ipairs(msgs) do
      ...
      snooze_pattern = "X%-Snooze%-Until: (%d+)"
      snooze_time = tonumber(string.match(snooze_header, snooze_pattern))
      if snooze_time <= os.time() then
